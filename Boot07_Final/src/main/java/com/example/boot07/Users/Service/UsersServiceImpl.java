...
import com.example.boot07.user.dao.UsersDao;
...
public class UsersServiceImpl implements UsersService {
   ...
   public void updateUserPwd(HttpSession session, UsersDto dto, Model model) {
      ...
      // 작업의 성공여부를 ModelAndView 객체에 담아놓는다(결국 HttpServletRequest에 담긴다)
   ...
   @Override
   public Map<String, Object> saveProfileImage(HttpServletRequest request, MultipartFile mf) {
      // 업로드된 파일에 대한 정보를 MultipartFile 객체를 이용해서 얻어낼수 있다.

      // 원본 파일명
      String orgFileName = mf.getOriginalFilename();
      //중복되지 않는 유일한 파일명을 구성한다
   ...
   public void deleteUser(HttpSession session, Model model) {
      ...
      //로그아웃 처리
      session.removeAttribute("id");
      //ModelAndView 객체에 탈퇴한 회원의 아이디 담기
      model.addAttribute("id", id);
   }
