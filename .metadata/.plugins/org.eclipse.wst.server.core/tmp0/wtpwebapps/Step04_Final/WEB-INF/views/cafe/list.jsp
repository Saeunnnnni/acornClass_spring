   ...
   <div class="container">
      ...
      <script>
         document.querySelectorAll(".pagination a").forEach(
            //item은 li의 참조값
            function(item){
               //모든 li 요소에 mouseover 이벤트가 발생했을 때 실행할 함수 등록
               item.addEventListener("mouseover", function(e){
                  //animation 클래스를 추가해서 animation이 동작하도록 한다
                  e.target.classList.add("animate__shakeY");
               });
               //모든 li 요소에 animationend 이벤트가 발생했을 때 실행할 함수 등록
               item.addEventListener("animationend", function(e){
                  //animation 클래스를 제거해서 다음 번에 추가되면 다시 animation이 동작되도록 한다
