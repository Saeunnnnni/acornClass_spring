...
<body>
   ...
   <script>
      ...
      document.querySelector("#submitBtn").addEventListener("click", ()=>{
    	  	//입력한 caption을 읽어온다.
    	  	const captipon=document.querySelector("#caption").value;
    	  	const files=document.querySelector("#image").files; 
    	  	//만일 caption 을 3글자 미망ㄴ으로 입력하거나 선택한 파일이 없다면 
    	  	if(cation.length < 3 || files.length == 0){
    	  		alert("caption 을 3글자 이상 입력하고 업로드할 이미지를 선택해주세요")
    	  		return;//함수를 여기서 종료
    	  	}
            ...
            const file=files[0];
           //image 라는 파라미터 명으로 현재 선택한 파일 객체를 담는다.
      ...
      dropZone.addEventListener("drop", (e)=>{
         ...
         if(files.length > 0){
            ...
            //선택한 파일의 정보를 input type="file" 요소에 넣기
